...
class SurrogateData:
	...
	def __init__(self,nCells,track,place_field_parameter,behavior,place_cell_probability):
		'''
		ToDo:
		...
		self.behavior = {
			...
			'trial_start': np.hstack([np.where(behavior['active'])[0][behavior['trials']['start'][:-1]],len(behavior['time_raw'])]),
		...
		self.field_activation = np.zeros((nCells,place_field_parameter['n_fields'],self.behavior['trial_ct']),'bool')

		self.tuning_curve_parameter = []
		for n in range(nCells):
					
			self.tuning_curve_parameter.append(self.set_PC_field(place_field_parameter,place_cell_probability))
	...
	def set_PC_field(self,place_field_parameter,place_cell_probability):
		...
		if self.options['place_cell_distribution'] == 'uniform':          ## uniform distribution
			
			tuning_curve_parameter['n_fields'] = random.randint(1,place_field_parameter['n_fields']) if (random.random()<place_cell_probability) else 0
			...
			for field in range(tuning_curve_parameter['n_fields']):
				...
				for key in ['theta','A','sigma','reliability']:
					tuning_curve_parameter['PF'][field][key] = draft_para(place_field_parameter[key])     ## uniform distribution
	...
	def generate_activity_all(self,nP=0):
		...
		if nP>0:
			...
			for ((S,act),n) in zip(results,range(self.nCells)):
				self.activity[n,:] = S
				self.field_activation[n,:self.tuning_curve_parameter[n]['n_fields'],:] = act
			...
			for n,tcp in tqdm.tqdm(enumerate(self.tuning_curve_parameter)):
				...
				self.activity[n,:],self.field_activation[n,:tcp['n_fields'],:] = generate_activity_fun(tcp,plt_bool=False)
	...
	def plot_activity(self,n):
		...
		intensity_model = lambda x,parameter,fields : intensity_model_from_position(x=x,parameter=parameter,n_x=self.behavior['nbin'],fields=fields)
		...
		dwelltime,_ = np.histogram(self.behavior['position'],bins=np.linspace(0,self.behavior['nbin'],self.behavior['nbin']+1))
		fmap = get_firingmap(self.activity[n,:],self.behavior['position'],dwelltime/15.)
		...
		for trial in range(self.behavior['trial_ct']):
			for f in range(self.tuning_curve_parameter[n]['n_fields']):
				...
				# print('t:',behavior['time'][[trial_starts[trial],trial_starts[trial+1]]])
					...
					ax_position.axvspan(self.behavior['time'][self.behavior['trial_start'][trial]],self.behavior['time'][self.behavior['trial_start'][trial+1]-1],color='tab:green',alpha=0.3)
		
		ax_position.plot(self.behavior['time'],self.behavior['position'],'k')
		ax_position.scatter(spike_times,self.behavior['position'][spike_frames],color='r',marker='o',s=2+2*spikes)
		...
		ax_fmap.plot(x,intensity_model(x,self.tuning_curve_parameter[n],range(self.tuning_curve_parameter[n]['n_fields'])),'k')
		ax_fmap.plot(fmap)
...
def generate_activity(tuning_curve_parameter,behavior,AP_mode='constant',plt_bool=False):
	'''
		generating a nonhomogeneous poisson process (pp) 
		on [0,T] with intensity function model_intensity
	'''

	intensity_model = lambda x,parameter,fields : intensity_model_from_position(x=x,parameter=parameter,n_x=behavior['nbin'],fields=fields)
	...
	if tuning_curve_parameter['n_fields'] > 0:
		...
		for field in tuning_curve_parameter['PF']:
			max_intensity = max(max_intensity,tuning_curve_parameter['A0']+field['A'])
	else:
	...
	if tuning_curve_parameter['n_fields']>0:
		...
		field_activation = np.zeros((tuning_curve_parameter['n_fields'],behavior['trial_ct']),'bool')
		...
		for trial in range(behavior['trial_ct']):
			...
			AP_idx_this_trial = np.where((AP_frame>=trial_start) & (AP_frame<trial_end))[0]
			...
			model_intensity_at_frame_this_trial = intensity_model(position_this_trial,tuning_curve_parameter,active_fields)

			model_intensity_at_spikes_this_trial = model_intensity_at_frame_this_trial[AP_frame[AP_idx_this_trial]-AP_frame[first_AP_idx]]        # evaluates intensity function at homogeneous pp points


			idx_keep_trial = np.random.rand(N_APs)<(model_intensity_at_spikes_this_trial/max_intensity)
...
def draft_para(in_range,n=1):
  	return in_range[0] + (in_range[1] - in_range[0])*random.random()#np.random.rand(n)
