...
def peak_method_bare(
    ...
    if plot:
        ...
        ax.axvline(PF_height, color="g", linestyle="--")
        ax.axvline(
...
def peak_method_wrapper(
    ...
    if "information" in methods:
        is_place_cell["information"] = information_method(
            behavior,
            activity,
            neurons=neurons,
            nbin=nbin,
    ...
    for neuron in tqdm.tqdm(neurons):
        ...
        is_place_cell["rates_active"][neuron], firing_threshold, significant_spikes = (
            get_firingrate(activity[neuron, :], f=15.0, sd_r=0, Ns_thr=10, prctile=10)
        )
...
def peak_method_batch(
    ...
    for neuron in tqdm.tqdm(neurons):
        is_place_cell[neuron] = peak_method(
            behavior,
            S[neuron, :],
            nbin=nbin,
...
def information_method_batch(
    ...
    for neuron in tqdm.tqdm(neurons):
        ...
        is_place_cell[neuron] = information_method(
            behavior,
            activity[neuron, :],
            fmap=fmaps[neuron, :] if fmaps else None,
            nbin=nbin,
            n_shuffles=n_shuffles,
            shuffle_trials=shuffle_trials,
            plot=plot,
...
def stability_method(
    behavior, activity, neurons=None, nbin=100, plot=False, ax=None, f=15.0
):
    """
    ...
    for neuron in tqdm.tqdm(neurons):
        ...
        for L, n_shuffle in enumerate(
            random.sample(range(activity.shape[0]), n_shuffles)
        ):
